...
struct LogLevelTests {
    ...
    @Test(arguments: zip(LogLevel.allCases, ["TRACE", "DEBUG", "INFO", "WARNING", "ERROR", "FATAL"]))
    func verifyDescription(level: LogLevel, label: String) {
        #expect(level.description == label)
    }

    @Test(arguments: LogLevel.allCases, LogLevel.allCases)
    func verifyComparison(left: LogLevel, right: LogLevel) {
        #expect(
